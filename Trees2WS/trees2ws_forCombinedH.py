      ...
      for s in systematics:
        for direction in ['Up','Down']:
          ...
          for var in systematicsVars:
            if var != "eventWeight": systematicsVarsDropWeight.append(var)
          aset = make_argset(ws,systematicsVarsDropWeight)
          
          h = ROOT.RooDataHist(hName,hName,aset)
          for row, weight in zip(sdf[mask][systematicsVarsDropWeight].to_numpy(),sdf[mask]["eventWeight"].to_numpy()): #rename weight to eventWeight
            for i, val in enumerate(row):
              aset[i].setVal(val)
            h.add(aset,weight)
